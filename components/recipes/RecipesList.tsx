...
export function RecipesList() {
  ...
  return (
    <div className="md:px-9 md:py-6">
      ...
      <div className="px-5 md:px-0 grid grid-cols-1 md:grid-cols-2 gap-3">
        {recipes.map((r) => (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="tnum text-xs text-ink-3 mt-0.5">
            {recipe.ingredients.length} ingredientes · {recipe.finalWeight} g totales
          </div>
        </div>
        <ChevronRight size={16} className="text-ink-3" />
      </div>
      <div className="grid grid-cols-4 gap-2 mt-3.5 pt-3 border-t border-border">
        <Stat label="kcal" value={Math.round(macros.kcal)} sub="/100g" />
